...
export default function SkillsScreen() {
  ...
  return <Screen>
    ...
    <View style={{ gap: 11 }}>{sorted.length ? sorted.map((row) => { const skill = relation(row.skills); const slug = skill?.slug; const skillName = skill?.name ?? "Skill"; const evidenceLabel = evidence(Number(row.reliability)); return <Pressable accessibilityRole="button" accessibilityLabel={`${skillName}. Score ${Math.round(Number(row.score))} out of 100. ${evidenceLabel}. ${row.attempts} observations.`} accessibilityHint="Opens a focused practice round" key={row.skill_id} disabled={!slug} onPress={() => slug && router.push({ pathname: "/(tabs)/train/practice/[slug]", params: { slug } })} style={({ pressed }) => ({ opacity: pressed ? .80 : 1 })}><Card style={{ borderColor: colors.lineStrong }}><View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}><View style={{ flex: 1, gap: 5 }}><Text style={{ color: colors.text, fontSize: 18, lineHeight: 24, fontWeight: "900" }}>{skillName}</Text><Text style={{ color: colors.muted, fontSize: 13.5, lineHeight: 19 }}>{evidenceLabel} · {row.attempts} observations</Text></View><View style={{ alignItems: "flex-end", gap: 2 }}><Text style={{ color: colors.cyan, fontSize: 22, fontWeight: "900", fontVariant: ["tabular-nums"] }}>{Math.round(Number(row.score))}</Text><Text style={{ color: colors.purple, fontSize: 13, lineHeight: 18, fontWeight: "900" }}>Practise →</Text></View></View><ProgressBar value={Number(row.score)} />{skill?.description ? <Body muted style={{ fontSize: 14, lineHeight: 20 }}>{skill.description}</Body> : null}</Card></Pressable>; }) : <Card><Title size={24}>Your profile is just beginning.</Title><Body muted>Complete the starting check to create your first skill scores.</Body><ActionLink label="Go to Train" hint="Open the training tab" onPress={() => router.push("/(tabs)/train")} /></Card>}</View>
  </Screen>;
}
