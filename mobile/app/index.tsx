...
export default function WelcomeScreen() {
  ...
  const heroHeight = fontScale > 1.35 ? 590 : 550;

  return <Screen atmospheric={false} contentStyle={{ gap: 16, paddingTop: Math.max(insets.top, 8) }}>
    <View style={{ marginHorizontal: -20, height: heroHeight, borderBottomLeftRadius: radius.xxl, borderBottomRightRadius: radius.xxl, overflow: "hidden", backgroundColor: colors.bgDeep, boxShadow: glow.hero }}>
      <HeroArtwork height={heroHeight} />
      <LinearGradient colors={["rgba(5,10,24,.10)", "rgba(8,16,38,.06)", "rgba(8,16,38,.56)", "#081026"]} locations={[0, .34, .67, 1]} style={{ position: "absolute", inset: 0 }} />
      ...
      <View style={{ position: "absolute", left: 22, right: 22, top: 14, gap: 4 }}><CogniLogo animated={false} /><Text style={{ color: "rgba(248,250,252,.82)", fontSize: 12.5, lineHeight: 19, ...typography.eyebrow, textTransform: "uppercase" }}>Knowledge moves you</Text></View>
      <View pointerEvents="none" accessible={false} style={{ position: "absolute", top: 112, alignSelf: "center", opacity: .95 }}><CogniMark size={168} /></View>
      <View style={{ position: "absolute", left: 22, right: 22, bottom: 26, gap: 10 }}><Title size={fontScale > 1.35 ? 36 : 42}>Sharpen how you think.</Title><Body style={{ maxWidth: 520, fontSize: 17, lineHeight: 25 }}>Real-life thinking practice for a calmer, clearer, more capable you.</Body><Text style={{ color: colors.cyan, fontSize: 12.5, lineHeight: 19, ...typography.eyebrow, textTransform: "uppercase" }}>A brighter you, by design.</Text></View>
    </View>
