...
export default async function SessionCompletePage() {
  ...
  if (session?.id) {
    ...
    if (responseIds.length) {
      ...
      if (updateRows?.length) {
        ...
        movements = (mappingRows ?? []).filter((row: any) => {
          const id = String(row.skill_id);
          if (seen.has(id)) return false;
          seen.add(id);
          return true;
        }).map((row: any) => ({ name: nestedSkillName(row.skills), before: null, after: currentBySkill.get(String(row.skill_id)) ?? 50, delta: null, touches: 1 }));
      }
    }
  }
  ...
  const alignment = total ? Math.round(responses.reduce((sum: number, row: any) => sum + Number(row.score_fraction ?? (row.is_correct ? 1 : 0)), 0) / total * 100) : 0;
  ...
  return (
    <div className="cg-mobile-page cg-results-screen">
      ...
      <p className="cg-results-scoreline">{fullyCorrect} fully correct • {alignment}% overall alignment across all formats.</p>

      <div className="cg-result-stats">
        <div><small>Alignment</small><strong>{alignment}%</strong></div>
        ...
        <div className="cg-session-skill-list">
          {movements.length ? movements.slice(0, 5).map((movement) => (
            <div className="cg-session-skill" key={movement.name}>
              <div><strong>{movement.name}</strong><small>{movement.touches > 1 ? `${movement.touches} questions` : "1 question"}</small></div>
              <div className="cg-session-skill-score">
                <strong>{Math.round(movement.after)}</strong>
                {movement.delta === null ? <span className="cg-delta neutral">trained</span> : <span className={`cg-delta ${movement.delta > 0 ? "up" : movement.delta < 0 ? "down" : "neutral"}`}>{movement.delta > 0 ? "+" : ""}{movement.delta.toFixed(1)}</span>}
              </div>
            </div>
      ...
      <section className="cg-card">
        ...
        {patterns.length ? <div className="cg-pattern-list">{patterns.map((item) => {
          const copy = patternCopy(item.pattern);
          return <div className="cg-pattern-row" key={item.pattern}><div><strong>{copy?.label ?? item.pattern.replaceAll("_", " ")}</strong><small>{copy?.action}</small></div><span className="cg-pill">×{item.count}</span></div>;
        })}</div> : <div className="cg-clean-session"><strong>No reasoning-error pattern detected.</strong><p>That does not prove mastery; it means this session did not generate a clear error signal.</p></div>}
      </section>

      <section className="cg-card cg-session-footer-card">
        <div><div className="cg-kicker">Total XP</div><strong>{profile?.xp ?? 0}</strong></div>
        <div><div className="cg-kicker">Next focus</div><strong>{focusSkill ?? "Keep gathering evidence"}</strong></div>
      </section>
