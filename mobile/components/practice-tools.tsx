...
import { Body, Card, Eyebrow, PrimaryButton, Title } from "./ui";
...
export function ToolkitShortcut() {
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel="Open saved ideas" accessibilityHint="Revisit your privately saved takeaways, even when offline" onPress={() => router.push("/toolkit")} style={({ pressed }) => ({ opacity: pressed ? .8 : 1, minHeight: 48 })}>
    <LinearGradient colors={["#212440", "#151c31"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={{ padding: 18, borderRadius: 20, borderWidth: 1, borderColor: colors.lineStrong, gap: 8 }}>
      <Eyebrow>Your thinking toolkit</Eyebrow><Title size={23}>{ready && data.ideas.length ? `${data.ideas.length} ${data.ideas.length === 1 ? "idea" : "ideas"} worth keeping` : "Keep what clicks."}</Title>
...
export function DailyLens({ audience }: { audience?: string | null }) {
  ...
  return <Card style={{ borderColor: "rgba(102,210,197,.38)", backgroundColor: "#101f2d", gap: 12 }}>
