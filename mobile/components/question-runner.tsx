...
function Choice({ label, selected, correct, wrong, onPress, disabled, prefix }: { label: string; selected?: boolean; correct?: boolean; wrong?: boolean; onPress: () => void; disabled?: boolean; prefix?: string }) {
  ...
  useEffect(() => {
    ...
    Animated.sequence([
      Animated.spring(scale, { toValue: correct ? 1.035 : .985, damping: 10, stiffness: 250, useNativeDriver: true }),
      Animated.spring(scale, { toValue: 1, damping: 12, stiffness: 230, useNativeDriver: true }),
    ]).start();
  }, [correct, wrong, scale]);
  ...
  return <Animated.View style={{ transform: [{ scale }] }}><Pressable accessibilityRole="button" accessibilityState={{ selected: Boolean(selected), disabled: Boolean(disabled) }} disabled={disabled} onPress={onPress} style={({ pressed }) => ({ minHeight: 60, borderWidth: 1, borderColor: border, backgroundColor: background, borderRadius: 19, borderCurve: "continuous", padding: 14, flexDirection: "row", alignItems: "center", gap: 12, opacity: pressed ? 0.78 : 1 })}>{prefix ? <LinearGradient colors={selected || correct ? ["rgba(0,229,255,.18)", "rgba(107,92,255,.24)"] : ["rgba(27,38,80,.9)", "rgba(19,27,59,.9)"]} style={{ width: 35, height: 35, borderRadius: 12, alignItems: "center", justifyContent: "center" }}><Text style={{ color: correct ? colors.green : colors.muted, fontSize: 14, fontWeight: "900" }}>{prefix}</Text></LinearGradient> : null}<Text style={{ flex: 1, color: colors.text, fontSize: 16.5, lineHeight: 23, fontWeight: "800" }}>{label}</Text>{correct ? <Text style={{ color: colors.green, fontSize: 20, fontWeight: "900" }}>✓</Text> : null}</Pressable></Animated.View>;
}
...
export function QuestionRunner({ mode, sessionId, challenges, answeredChallengeIds = [], modeLabel, onComplete }: Props) {
  ...
  return <Screen>
    <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", gap: 12 }}><View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}><CogniMark size={26} animated={false} /><Eyebrow>{modeLabel ?? (mode === "diagnostic" ? "Starting check" : mode === "practice" ? "Skill practice" : "Daily questions")}</Eyebrow></View><Text style={{ color: colors.muted, fontSize: 14, fontWeight: "800", fontVariant: ["tabular-nums"] }}>{Math.min(answered.size + 1, challenges.length)} of {challenges.length}</Text></View>
    <ProgressBar value={progress} />
    {challenge.scenario_context ? <Card style={{ backgroundColor: "rgba(0,229,255,.04)", borderColor: "rgba(0,229,255,.20)" }}><Body muted>{challenge.scenario_context}</Body></Card> : null}
    <Card><View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}><Eyebrow>{formatLabels[type] ?? "Question"}</Eyebrow><Text style={{ color: colors.purple, fontSize: 12.5, fontWeight: "900" }}>{difficulty(challenge.difficulty)}</Text></View><Title size={28}>{challenge.title}</Title><Body>{challenge.prompt}</Body></Card>
    {renderAnswers()}
    {challenge.confidence_required && !result ? <Card><View style={{ flexDirection: "row", justifyContent: "space-between" }}><Text style={{ color: colors.text, fontSize: 16, fontWeight: "900" }}>How sure are you?</Text><Text style={{ color: colors.cyan, fontWeight: "900" }}>{confidence}%</Text></View><View style={{ flexDirection: "row", gap: 8 }}>{[40,60,80,100].map((value) => <Pressable key={value} onPress={() => setConfidence(value)} style={{ flex: 1, minHeight: 44, borderRadius: 13, borderWidth: 1, borderColor: confidence === value ? colors.violet : colors.line, backgroundColor: confidence === value ? "rgba(107,92,255,.18)" : colors.panel2, justifyContent: "center", alignItems: "center" }}><Text style={{ color: colors.text, fontWeight: "800" }}>{value}</Text></Pressable>)}</View></Card> : null}
    {error ? <Text selectable style={{ color: "#ff9ac0", fontSize: 15, lineHeight: 22 }}>{error}</Text> : null}
    {!result ? <PrimaryButton label={busy ? "Checking…" : type === "ranking" ? "Use this order" : type === "classification" ? "Check my groups" : type === "multi_select" ? "Submit answers" : "Submit answer"} onPress={() => void submit()} disabled={!ready || busy} /> : <View style={{ gap: 12 }}><Card style={{ borderColor: result.correct ? "rgba(34,211,164,.58)" : score >= .5 ? "rgba(255,176,32,.5)" : "rgba(236,72,153,.44)" }}><View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", gap: 12 }}><View style={{ flex: 1 }}><Eyebrow>{result.correct ? "Correct" : score >= .5 ? "Partly right" : "Worth reviewing"}</Eyebrow><Title size={25}>{result.correct ? "Good choice" : score >= .5 ? "Nearly there" : "Now you’ve seen it"}</Title></View><LinearGradient colors={result.correct ? ["rgba(34,211,164,.22)", "rgba(0,229,255,.12)"] : ["rgba(107,92,255,.2)", "rgba(184,85,255,.16)"]} style={{ paddingHorizontal: 12, paddingVertical: 8, borderRadius: 999 }}><Text style={{ color: colors.cyan, fontSize: 15, fontWeight: "900" }}>+{result.xpEarned} XP</Text></LinearGradient></View></Card><Body>{result.explanation}</Body>{result.skillUpdates?.length ? <Card><Eyebrow>Skill movement</Eyebrow>{result.skillUpdates.map((update) => <View key={update.slug} style={{ flexDirection: "row", justifyContent: "space-between", gap: 14 }}><Text style={{ flex: 1, color: colors.text, fontSize: 16, lineHeight: 22, fontWeight: "800" }}>{(update.name ?? update.slug).replace(/^./, (c) => c.toUpperCase())}</Text><Text style={{ color: Number(update.delta ?? 0) >= 0 ? colors.green : colors.pink, fontSize: 16, fontWeight: "900" }}>{Number(update.delta ?? 0) >= 0 ? "+" : ""}{Number(update.delta ?? 0).toFixed(1)}</Text></View>)}</Card> : null}<Card><Eyebrow>Key idea</Eyebrow><Body>{result.thinkingPrinciple}</Body></Card><Card><Eyebrow>Why this matters with AI</Eyebrow><Body>{result.application}</Body></Card><PrimaryButton label="Keep going →" onPress={() => void next()} /></View>}
  </Screen>;
}
