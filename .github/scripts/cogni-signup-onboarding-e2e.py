#!/usr/bin/env python3
"""Verify signup, onboarding validation and account deletion on Android."""
...
BASE_EMAIL = os.environ["E2E_EMAIL"]
PASSWORD = os.environ["E2E_PASSWORD"]
EMAIL = BASE_EMAIL.replace(".route.", ".signup.") if ".route." in BASE_EMAIL else f"signup.{BASE_EMAIL}"
API_LEVEL = os.environ.get("API_LEVEL", "unknown")
OUT = Path(f"/tmp/cogni-auth-route-api-{API_LEVEL}/signup-onboarding")
...
def dump_ui(label: str) -> list[Node]:
    remote = "/sdcard/cogni-signup-window.xml"
...
def main() -> int:
    ...
    # Every primary action must respond, even before the form is complete.
    tap("Create account", scroll=True)
    wait_for("Enter your email address.")
    input_text("Name", "Cogni Signup E2E")
    input_text("Email", EMAIL)
    tap("Create account", scroll=True)
    wait_for("Use at least 8 characters for your password.")
    ...
    # Account deletion is part of the real signed-in account lifecycle and also
    # cleans this disposable signup account from production.
    tap("Profile")
    wait_for("Cogni Signup E2E", timeout=45)
    tap("Delete account", scroll=True)
    wait_for("Delete Cogni account?", timeout=20)
    tap("Delete account")
    scroll_to_top()
    wait_for("Sharpen how you think.", timeout=60)
    assert_absent("Reset your password")

    capture("pass")
    print("PASS: signup validation, account creation, onboarding validation, context save, Train readiness and account deletion all worked.")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"SIGNUP / ONBOARDING E2E FAILURE: {error}")
        capture("failure")
        raise
