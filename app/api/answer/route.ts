...
function evaluateAnswer(
  ...
  if (interactionType === "multi_select") {
    ...
    for (let index = 0; index < optionCount; index += 1) {
      if (selected.has(index) === correctSet.has(index)) decisionsRight += 1;
    }
  ...
  if (interactionType === "ranking") {
    ...
    const chosen = responsePayload.map(Number).filter(Number.isInteger);
    ...
    const rightPositions = chosen.reduce((count, value, index) => count + (value === expected[index] ? 1 : 0), 0);
    const fraction = optionCount ? rightPositions / optionCount : 0;
    return { correct: arraysEqual(chosen, expected), scoreFraction: fraction, storedPayload: chosen };
  }
  ...
  if (interactionType === "classification") {
    ...
    const chosen = responsePayload as Record<string, string>;
    ...
    const right = keys.reduce((count, key) => count + (chosen[key] === expected[key] ? 1 : 0), 0);
    const fraction = keys.length ? right / keys.length : 0;
...
export async function POST(req: Request) {
  try {
    ...
    if (body.mode === "training") {
      const { data: trainingSession } = await admin.from("training_sessions").select("id,status").eq("id", body.sessionId).eq("user_id", user.id).maybeSingle();
      if (!trainingSession) return NextResponse.json({ error: "Training session not found." }, { status: 403 });
      if (trainingSession.status === "completed") return NextResponse.json({ error: "This training session is already complete." }, { status: 409 });
    ...
    if (body.mode === "training" && challenge.is_diagnostic) return NextResponse.json({ error: "Diagnostic challenges cannot be submitted as training." }, { status: 400 });
    ...
    const evaluation = evaluateAnswer(
      ...
      key.correct_index ?? null,
      key.correct_answer ?? key.correct_index,
    );
    ...
    const pattern = evaluation.correct
      ? null
      : interactionType === "single_choice" || interactionType === "triage"
    ...
    for (const mapping of mappings ?? []) {
      ...
      const scoreAfter = nextSkillScoreObserved(scoreBefore, challenge.difficulty, scoreFraction, Boolean(challenge.is_diagnostic), mapping.weight ?? 1);
    ...
    if (pattern) {
      const { data: errorPattern } = await admin.from("user_error_patterns").select("id,count").eq("user_id", user.id).eq("pattern", pattern).maybeSingle();
      if (errorPattern) await admin.from("user_error_patterns").update({ count: errorPattern.count + 1, last_seen_at: new Date().toISOString() }).eq("id", errorPattern.id);
      else await admin.from("user_error_patterns").insert({ user_id: user.id, pattern, count: 1 });
    }

    const today = localDateKey();
    const yesterday = previousLocalDateKey();
    const { data: profile } = await admin.from("profiles").select("xp,current_streak,last_session_date").eq("id", user.id).single();
    let streak = profile?.current_streak ?? 0;
    if (profile?.last_session_date !== today) streak = profile?.last_session_date === yesterday ? streak + 1 : 1;
    await admin.from("profiles").update({ xp: (profile?.xp ?? 0) + xp, current_streak: streak, last_session_date: today }).eq("id", user.id);

    await admin.from("analytics_events").insert([
      { user_id: user.id, event_name: "answer_submitted", properties: { challenge_id: body.challengeId, interaction_type: interactionType, score_fraction: scoreFraction, mode: body.mode, session_id: body.sessionId, confidence: body.confidence, response_time_ms: body.responseTimeMs, xp_awarded: xp } },
      { user_id: user.id, event_name: evaluation.correct ? "answer_correct" : "answer_incorrect", properties: { challenge_id: body.challengeId, interaction_type: interactionType, score_fraction: scoreFraction, mode: body.mode, session_id: body.sessionId } },
    ]);
