...
export default function PracticeScreen() {
  ...
  const load = useCallback(async () => {
    if (!slug) { setError("Skill not found."); setLoading(false); return; }
    setLoading(true); setError("");
    try { setData(await apiFetch<PracticePayload>(`/api/mobile/practice/${encodeURIComponent(slug)}`)); }
    catch (caught) {
      if (caught instanceof ApiError && caught.code === "premium_required") {
        router.replace({ pathname: "/paywall", params: { feature: "focused_practice", source: "server_gate" } });
        return;
      }
