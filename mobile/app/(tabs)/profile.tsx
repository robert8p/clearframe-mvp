...
import { Body, Card, Eyebrow, ErrorState, LoadingState, Pill, PrimaryButton, Screen, Title } from "@/components/ui";
...
export default function ProfileScreen() {
  ...
  return <Screen>
    <LinearGradient colors={["rgba(38,43,112,.95)", "rgba(15,20,51,.98)"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={{ borderRadius: 28, borderWidth: 1, borderColor: colors.line, padding: 20, alignItems: "center", gap: 10 }}>
      ...
      <Title size={27}>{name || "Your Cogni profile"}</Title><Text selectable style={{ color: colors.muted, fontSize: 14.5 }}>{data.profile.email}</Text>{data.profile.audience_segment ? <Pill accent>{audienceNames[data.profile.audience_segment] ?? data.profile.audience_segment}</Pill> : null}
    ...
    <Card><View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", gap: 8 }}><Eyebrow>Milestones</Eyebrow><Text style={{ color: colors.soft, fontSize: 12.5 }}>Built from real progress</Text></View><View style={{ flexDirection: "row", flexWrap: "wrap", gap: 9 }}>{milestones.map((item) => <View accessible accessibilityLabel={`${item.label}. ${item.unlocked ? "Unlocked" : "Locked"}.`} key={item.label} style={{ flexGrow: 1, flexBasis: 96, minHeight: 88, borderRadius: 18, borderWidth: 1, borderColor: item.unlocked ? "rgba(107,92,255,.65)" : colors.line, backgroundColor: item.unlocked ? "rgba(107,92,255,.13)" : "rgba(16,23,53,.7)", alignItems: "center", justifyContent: "center", gap: 6, opacity: item.unlocked ? 1 : .58 }}><Text style={{ fontSize: 22 }}>{item.icon}</Text><Text style={{ color: colors.text, fontSize: 12.5, lineHeight: 17, textAlign: "center", fontWeight: "800" }}>{item.label}</Text></View>)}</View></Card>
