...
export default function LessonScreen() {
  ...
  useEffect(() => { setThought(""); setRevealed(false); setFeedbackSent(false); setFeedbackError(""); }, [today?.lesson?.id]);
  ...
  async function complete() { if (completionLock.current) return; completionLock.current = true; setBusy(true); setError(""); try { await apiFetch<{ ok: boolean; xpEarned: number }>("/api/mobile/lesson/complete", { method: "POST", body: JSON.stringify({ lessonId: lesson.id }) }); router.replace("/(tabs)/train/session"); } catch (caught) { setError(caught instanceof Error ? caught.message : "Could not finish this insight."); } finally { completionLock.current = false; setBusy(false); } }
  async function markNotRelevant() { if (feedbackLock.current || feedbackSent) return; feedbackLock.current = true; setFeedbackBusy(true); setFeedbackError(""); try { await apiFetch<{ ok: boolean }>("/api/mobile/context-feedback", { method: "POST", body: JSON.stringify({ lessonId: lesson.id }) }); setFeedbackSent(true); } catch (caught) { setFeedbackError(caught instanceof Error ? caught.message : "Could not save that feedback."); } finally { feedbackLock.current = false; setFeedbackBusy(false); } }
  function keepEntryVisible() { setTimeout(() => scrollRef.current?.scrollToEnd({ animated: !reducedMotion }), 260); }
  function reveal() { Keyboard.dismiss(); setRevealed(true); }
  return <Screen ref={scrollRef} contentStyle={{ paddingBottom: 180 }}>
    ...
    {lesson.scenario_context ? <Card style={{ borderColor: "rgba(0,229,255,.28)" }}><Eyebrow>{today.situationLabel ?? "Situation"}</Eyebrow><Body>{lesson.scenario_context}</Body>{feedbackSent ? <Text accessibilityLiveRegion="polite" style={{ color: colors.soft, fontSize: 13.5, lineHeight: 20, fontWeight: "700" }}>Thanks — your feedback is saved.</Text> : <View style={{ alignItems: "flex-start" }}><ActionLink label={feedbackBusy ? "Saving…" : "Not relevant to me"} onPress={() => void markNotRelevant()} hint="Tell Cogni this situation does not fit your context" /></View>}{feedbackError ? <Text accessibilityLiveRegion="assertive" selectable style={{ color: colors.danger, lineHeight: 22 }}>{feedbackError}</Text> : null}</Card> : null}
    ...
    <Card style={{ borderColor: "rgba(255,79,216,.34)" }}><Eyebrow>Your turn</Eyebrow><Title size={25}>What do you think?</Title><Body>{lesson.content.try_it}</Body><FormField label="Your reflection" hint="One sentence is enough. Your text is not sent or saved. You can also reflect without typing." multiline value={thought} onChangeText={setThought} onFocus={keepEntryVisible} placeholder="Write your thought here" placeholderTextColor={colors.soft} />{!revealed ? <><PrimaryButton label="Reveal the thinking move" disabled={thought.trim().length < 3} onPress={reveal} /><PrimaryButton secondary label="I’ve reflected without typing" onPress={reveal} /></> : null}</Card>
    {error ? <Text accessibilityLiveRegion="assertive" selectable style={{ color: colors.danger, lineHeight: 22 }}>{error}</Text> : null}
    {revealed ? <><Card style={{ borderColor: "rgba(34,211,164,.52)" }}><Eyebrow>The reveal</Eyebrow><Title size={23}>The thinking move</Title><Body>{lesson.content.reveal}</Body></Card><Card><Eyebrow>Why this matters with AI</Eyebrow><Body>{lesson.content.ai_age}</Body></Card><PrimaryButton label={busy ? "Saving…" : "Complete insight +5 XP"} disabled={busy} onPress={() => void complete()} /></> : null}
  </Screen>;
}
