...
export default function TrainHomeScreen() {
  ...
  if (today?.state === "diagnostic") { const answered = today.answeredChallengeIds?.length ?? 0; const progress = questionCount ? answered / questionCount * 100 : 0; return <Screen refreshing={refreshing} onRefresh={() => void load(true)}><Hero eyebrow="Starting check" title="Find your best starting point" body={`${questionCount} short decisions across evidence, reasoning, AI verification, bias and uncertainty. Plan for about 4–6 minutes.`} cta={answered ? "Continue starting check" : "Start your check"} onPress={() => router.push("/(tabs)/train/session")} progress={progress} /><Card><Eyebrow>Why this comes first</Eyebrow><Body>Cogni uses a shared core plus situations relevant to your context so your scores stay comparable while the experience feels useful.</Body><Body muted style={{ fontSize: 14, lineHeight: 20 }}>This is not pass/fail. Early scores are provisional and gain weight only as Cogni collects more evidence.</Body></Card></Screen>; }
  if (today?.state === "training") { const answered = today.session?.answeredChallengeIds?.length ?? 0; const progress = questionCount ? answered / questionCount * 100 : 0; return <Screen refreshing={refreshing} onRefresh={() => void load(true)}><Hero eyebrow={today.modeLabel ?? "Today’s training"} title={answered ? "Keep the momentum going" : "Your decisions are ready"} body={answered ? `${answered} of ${questionCount} questions complete.` : `${questionCount} situations chosen around the skills that will give you the most value today. Plan for about 3–5 minutes.`} cta={answered ? "Continue training" : "Train now"} onPress={() => router.push("/(tabs)/train/session")} progress={progress} /><Card><Eyebrow>Why these situations</Eyebrow><Body muted>Cogni uses your learning context, recent skill evidence and recent scenario history to choose relevant practice. Context changes the situation—not the standard of thinking expected from you.</Body></Card></Screen>; }
  if (today?.state === "complete") return <Screen refreshing={refreshing} onRefresh={() => void load(true)}><Hero eyebrow="Daily goal complete" title="Judgement strengthened" body={needsProForFocusedPractice ? "Today’s free core session is done. Cogni Pro unlocks unlimited additional focused practice whenever you want it." : "Today’s core session is done. Training stays open whenever you want another focused round."} cta={needsProForFocusedPractice ? "Unlock more practice" : "Choose a skill to practise"} onPress={() => needsProForFocusedPractice ? openPaywall("daily_complete", "focused_practice") : router.push("/(tabs)/skills")} progress={100} /><Card><Eyebrow>{needsProForFocusedPractice ? "Free habit complete" : "Keep building"}</Eyebrow><Body>{needsProForFocusedPractice ? "Come back tomorrow for the next free core session, or upgrade to choose a skill and keep training now." : "Pick the skill you want to work on next. Cogni will prefer questions you haven’t seen before."}</Body></Card></Screen>;
