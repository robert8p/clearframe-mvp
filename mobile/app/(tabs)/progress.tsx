...
export default function ProgressScreen() {
  ...
  return <Screen>
    ...
    {nextSkill && nextSkillSlug ? <Card style={{ borderColor: "rgba(0,229,255,.28)" }}><Eyebrow>Next best move</Eyebrow><Title size={24}>Sharpen {nextSkill.name}</Title><Body muted>This is currently your lowest measured skill. A focused round gives Cogni more evidence while practising the area with the clearest upside.</Body><PrimaryButton label={`Practise ${nextSkill.name}`} onPress={() => router.push({ pathname: "/(tabs)/train/practice/[slug]", params: { slug: nextSkillSlug } })} /></Card> : null}
