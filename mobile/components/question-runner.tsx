...
export function QuestionRunner({ mode, sessionId, challenges, answeredChallengeIds = [], modeLabel, onComplete }: Props) {
  ...
  function renderAnswers() {
    if (type === "multi_select") return <View style={{ gap: 10 }}>
      ...
      {challenge.options.map((option, optionIndex) => {
        ...
        return <Choice key={optionIndex} label={option} selected={active} correct={right} wrong={wrong} disabled={Boolean(result) || busy} prefix={active ? "✓" : ""} onPress={() => {
    ...
    if (type === "ranking") {
      ...
      return <View style={{ gap: 10 }}>{ranking.map((optionIndex, rankIndex) => <Choice key={`rank-${optionIndex}`} label={challenge.options[optionIndex]} prefix={String(rankIndex + 1)} disabled={Boolean(result) || busy} onPress={() => choose(() => setRanking((current) => current.filter((value) => value !== optionIndex)))} />)}{!result ? remaining.map(({ option, optionIndex }) => <Choice key={optionIndex} label={option} prefix="+" disabled={busy} onPress={() => choose(() => setRanking((current) => [...current, optionIndex]))} />) : null}{result ? <Card><Eyebrow>Best order</Eyebrow>{correctList.map((optionIndex, rankIndex) => <Body key={`${optionIndex}-${rankIndex}`}>{rankIndex + 1}. {challenge.options[optionIndex]}</Body>)}</Card> : null}</View>;
    }
  ...
  if (showSummary) return <Screen><Card style={{borderColor:colors.cyan}}><Eyebrow>Practice complete</Eyebrow><Title size={30}>Take one idea with you.</Title><Body>Your submitted answers are saved. A useful next step is to notice where this thinking applies today.</Body></Card><Card><Eyebrow>Keep this in mind</Eyebrow><Body>{result?.thinkingPrinciple || "Pause on an assumption, then ask what evidence would change your mind."}</Body></Card><Body muted>Progress comes from understanding the reasoning, not chasing a perfect score.</Body>{error ? <Text accessibilityLiveRegion="assertive" style={{color:colors.danger}}>{error}</Text> : null}<PrimaryButton label="Back to training" trailingArrow loading={advancing} onPress={()=>void finish()} /></Screen>;
  const score = result?.scoreFraction ?? (result?.correct ? 1 : 0);
  return <Screen ref={scrollRef}>
    ...
    {renderAnswers()}
    {challenge.confidence_required && !result ? <Card><View style={{ flexDirection: "row", justifyContent: "space-between", gap: 8 }}><Text style={{ color: colors.text, fontSize: 16, fontWeight: "900" }}>How sure are you?</Text><Text style={{ color: colors.cyan, fontWeight: "900" }}>{confidence}%</Text></View><View accessibilityRole="radiogroup" style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>{[40,60,80,100].map((value) => <Pressable accessibilityRole="radio" accessibilityLabel={`${value} percent confident`} accessibilityState={{ checked: confidence === value, disabled: busy }} disabled={busy} key={value} onPress={() => choose(() => setConfidence(value))} style={{ flexGrow: 1, flexBasis: 58, minHeight: 48, paddingVertical: 12, borderRadius: 14, borderWidth: 1, borderColor: confidence === value ? colors.violet : colors.lineStrong, backgroundColor: confidence === value ? "rgba(107,92,255,.18)" : colors.panel2, justifyContent: "center", alignItems: "center" }}><Text style={{ color: colors.text, fontWeight: "800" }}>{value}</Text></Pressable>)}</View></Card> : null}
