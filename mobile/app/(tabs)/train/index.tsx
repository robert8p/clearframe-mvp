...
export default function TrainHomeScreen() {
  ...
  const load = useCallback(async (refresh = false) => { refresh ? setRefreshing(true) : setLoading(true); setError(""); try { setToday(await apiFetch<TodayResponse>("/api/mobile/today")); } catch (caught) { setError(caught instanceof Error ? caught.message : "Could not load training."); } finally { setLoading(false); setRefreshing(false); } }, []);
