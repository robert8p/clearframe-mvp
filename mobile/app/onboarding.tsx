...
export default function OnboardingScreen() {
  ...
  const [functionArea, setFunctionArea] = useState(""); const [industry, setIndustry] = useState(""); const [goal, setGoal] = useState("");
  ...
  useEffect(() => {
    ...
    apiFetch<MobileProfileResponse>("/api/mobile/profile").then((data) => {
      ...
      setFunctionArea(data.profile.function_area ?? ""); setIndustry(data.profile.industry ?? ""); setGoal(data.profile.primary_goal ?? "");
      setStudyStage(data.profile.study_stage ?? ""); setResponsibilityScope(data.profile.responsibility_scope ?? ""); setOrganisationScale(data.profile.organisation_scale ?? "");
  ...
  const isCasual = selectedAudience === "casual"; const isStudent = selectedAudience === "university_student";
  ...
  function chooseAudience(next: MobileAudience) {
    ...
    setAudience(next); setFunctionArea(""); setIndustry(""); setGoal(""); setStudyStage(""); setResponsibilityScope(""); setOrganisationScale("");
  }

  async function save() {
    if (!selectedAudience || busy) return;
  ...
  return <Screen>
    ...
    <LinearGradient colors={["rgba(30,40,96,.95)", "rgba(13,19,48,.98)"]} style={{ borderRadius: 26, borderWidth: 1, borderColor: colors.line, padding: 18, gap: 7 }}>
      <Eyebrow>Make Cogni relevant to you</Eyebrow><Title size={29}>What kind of learning context fits you?</Title>
    ...
    <View accessibilityRole="radiogroup" style={{ gap: 10 }}>
      {MOBILE_AUDIENCES.map((item) => <Pressable accessibilityRole="radio" accessibilityLabel={item.label} accessibilityHint={item.text} accessibilityState={{ selected: audience === item.slug }} key={item.slug} onPress={() => chooseAudience(item.slug)} style={({ pressed }) => ({ opacity: pressed ? .80 : 1 })}>
        <View style={{ minHeight: 88, padding: 14, borderRadius: 21, borderCurve: "continuous", borderWidth: 1, borderColor: audience === item.slug ? colors.cyan : colors.lineStrong, backgroundColor: audience === item.slug ? "rgba(107,92,255,.16)" : "rgba(16,23,53,.92)", flexDirection: "row", alignItems: "center", gap: 13 }}>
          <LinearGradient colors={audience === item.slug ? ["rgba(0,229,255,.20)", "rgba(184,85,255,.24)"] : ["rgba(32,43,85,.9)", "rgba(20,28,60,.9)"]} style={{ width: 48, height: 48, borderRadius: 16, alignItems: "center", justifyContent: "center" }}><Text style={{ fontSize: 23 }}>{item.icon}</Text></LinearGradient>
    ...
    {selectedAudience ? <Card>
      <Eyebrow>Optional — tailor scenarios further</Eyebrow>
      <Body muted style={{ fontSize: 14, lineHeight: 20 }}>{isCasual ? "Choose the everyday areas you care about so Cogni can favour situations that feel useful." : "These details let Cogni match examples to your real context instead of guessing from free text."}</Body>
      <View style={{ gap: 18 }}>
