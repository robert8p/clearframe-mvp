...
type SkillRow = { name: string; slug: string };
...
export async function GET(req: Request, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const skillQuery = await supabase.from("skills").select("name,slug").eq("slug", slug).maybeSingle();
    const skill = skillQuery.data as SkillRow | null;
