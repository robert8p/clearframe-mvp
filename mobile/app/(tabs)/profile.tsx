...
export default function ProfileScreen() {
  ...
  return <Screen>
    ...
    <Card><Eyebrow>Personalisation</Eyebrow><Body muted>{isCasual?"Keep your interests and learning goal current so Cogni can favour useful everyday situations.":"Keep these details current so Cogni can choose examples closer to your real decisions."}</Body><View style={{ gap: 14 }}><FormField label="Name" value={name} onChangeText={setName} placeholder="Your name" placeholderTextColor={colors.soft} /><FormField label={isCasual?"Interest area":isStudent?"Study area":"Function / discipline"} value={functionArea} onChangeText={setFunctionArea} placeholder={isCasual?"e.g. AI, news, purchases, personal growth":"Optional"} placeholderTextColor={colors.soft} />{!isCasual?<FormField label="Industry" value={industry} onChangeText={setIndustry} placeholder="Optional" placeholderTextColor={colors.soft} />:null}<FormField label={isCasual?"What would you like to get better at?":"Primary goal"} value={goal} onChangeText={setGoal} placeholder={isCasual?"e.g. Judge online claims more clearly":"What do you most want to improve?"} placeholderTextColor={colors.soft} /></View>{saved ? <Text accessibilityLiveRegion="polite" style={{ color: colors.green, fontWeight: "800", lineHeight: 22 }}>{saved}</Text> : null}{error ? <Text accessibilityLiveRegion="assertive" selectable style={{ color: colors.danger, lineHeight: 22 }}>{error}</Text> : null}<PrimaryButton label={busy ? "Saving…" : "Save profile"} disabled={busy} onPress={() => void save()} /></Card>
