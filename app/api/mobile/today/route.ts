...
export async function GET(req: Request) {
  try {
    ...
    const session = await getOrCreateDailyTrainingSession(supabase, user.id);
    ...
    if (!lessonComplete) {
      ...
      if (lesson) return NextResponse.json({ state: "lesson", profile, session, lesson, modeLabel: audienceSessionLabel(profile.audience_segment) });
    }
